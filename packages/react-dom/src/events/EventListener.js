...
//如果又确实调用了的话，浏览器只会console一个warning，而不会真的去执行preventDefault方法。
//根据规范，默认值为false. 但是chrome, Firefox等浏览器为了保证滚动时的性能，
//在document-level nodes(Window, Document, Document.body)上
//针对touchstart和touchmove事件将passive默认值改为了true，
//保证了在页面滚动时不会因为自定义事件中调用了preventDefault而阻塞页面渲染。
